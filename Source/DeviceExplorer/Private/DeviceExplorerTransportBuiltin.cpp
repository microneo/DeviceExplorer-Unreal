...
private:
	...
	{
		// Copied first: a handler is allowed to close the transport, which clears
		// Callbacks while this handler is still on the stack.
		if (const TFunction<void(const FString&)> Handler = Callbacks.OnMessage)
		{
			Handler(DeviceExplorerUtf8ToString(Payload.data(), Payload.size()));
		}
	}
	...
	void Fail(const FString& Reason)
	{
		...
		if (bWasConnected)
		{
			...
			{
				Handler(1006, Reason, false);
			}
		}
		else if (const TFunction<void(const FString&)> Handler = Callbacks.OnConnectionError)
		{
			Handler(Reason);
		}
	}
